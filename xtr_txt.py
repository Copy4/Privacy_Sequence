...
class xtr_txt():
    ...
    def ban_check(self,url,list_url):        # Rejet des urls trouvées sur les pages visitées
        ...
        for word in self.ban_word:
            if word in url:         #Si l'url contient un mot clé impertinent
                return True
        if url[0:5] != 'https':     #Si l'url n'est pas une url sécurisée ou non associer au bon protocole
            return True
        elif url.count('/')>5:      #Si l'url est trop profonde dans l'arborescence du site
            return True
    ...
    def run(self):
        ...
        driver = self.creat_browser()

        # Liste des sites déjà visités (que l'on va donc éviter de re-visiter)
        liste_URL_trouvees = self.load_progress()
        liste_URL_visites = self.load_visited()
        # Liste des sites qui seront visités durant cette saission (initialement vide car on n'a pas encore commencé)
        Liste_nouveaux_URL_visite = []

        # Initialisation de la liste des sites à visiter
        if len(liste_URL_trouvees) >= 5:
            pass
        else:
            liste_URL_trouvees = self.defaut
            liste_URL_trouvees = self.defaut
        # Algorythme en lui même commence ici
        ...
        while len(liste_URL_trouvees) > 0: # tant qu'il y a des sites à visiter
            ...
            print('URL =', URL )
            if (not(URL in URL_list_set) or URL in self.defaut) and URL[0:5] == 'https' : # si on a jamais regardé ce site
                ...
                print('Liste des urls a traiter',liste_URL_trouvees )
            compteur += 1

            print('compteur :'+' '+str(compteur))
            if compteur >= 100:
                self.save_progress(liste_URL_trouvees)
                self.save_visited(liste_URL_visites)
                liste_URL_trouvees = self.load_progress()
                liste_URL_visites = self.load_visited()
                compteur = 0
    ...
    def save(self,URL,txts):
        ...
        txt_final = txt_final + '\n'
        print(txt_final)
        doc = open(self.endroit+'/'+'tampon_bdd3.txt','a', encoding="utf-8")
        doc.write(txt_final)
        doc.close()

    def save_progress(self,liste_nouveaux_URL):
    ...
    def load_progress(self):
        doc = open(self.endroit + '/' + 'sites_web_trouves.txt', 'r')
        str_doc = doc.read()
        liste_URL_visites = str_doc.split('\n')
        return liste_URL_visites

    def load_visited(self):
        doc = open(self.endroit + '/' + 'sites_web_visites.txt', 'r')
        str_doc = doc.read()
        liste_URL_visites = str_doc.split('\n')
        return liste_URL_visites
